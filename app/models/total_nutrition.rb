class TotalNutrition
  ...
  def calc_intake_total(foods)
    calc_cal_total(foods)
    calc_nutritions_total(foods)
  end
  ...
  private
    ...
    def calc_cal_total(foods)
      sum = 0
      foods.each do |f|
        sum += f.calorie * f.reference_amount
      end
      self.attributes = { calorie: sum.floor(4) }
    end

    def calc_nutritions_total(foods)
      params = attributes
